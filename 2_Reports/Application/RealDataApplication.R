...
###############
# Older code
test <- mgcv::gam(mwbi ~ s(as.numeric(sp_wd), bs = "cs", k = 3), data = filterNA)
summary(test)
summary(lm(mwbi ~ as.numeric(sp_wd), data = filterNA))
hist(filterNA$sp_wd, breaks = 9)
...
select(mwbi, sp_wdF) %>% mutate(GroupID = paste('G', sp_wdF, sep = '')) %>% select(-sp_wdF)

data.frame(mwbi = te2$mwbi, model.matrix(mwbi ~ GroupID - 1, data=te2))
colnames(te3) <- c('mwbi', paste('G', 1:9, sep = ''))

data.frame(mwbi = subsetData$mwbi, model.matrix(mwbi ~ GroupID - 1, data=subsetData))


pim(PO(L(mwbi), R(mwbi)) ~ I(R(G3) * L(G1)), data = te3, compare = 'unique', model = "customized")

test <- sample_n(filterNA, 1000, replace = FALSE) %>% filter(complete.cases(.))
pim(mwbi ~ sp_wd, data = test, link = 'probit', model = 'difference')
pnorm(-0.04589, lower.tail = TRUE)
pnorm(-0.32123, lower.tail = TRUE)
as.factor(test$sp_wd)
pim(mwbi ~ as.factor(sp_wd), data = test, link = 'probit', model = 'difference')

#test <- sample_n(dataWB, 250, replace = FALSE) %>% filter(complete.cases(.))
pim(formula = mwbi ~ sp_wd + male + minority + deprived, data = test, link = 'probit', model = "marginal")

test$sp_wdF <- factor(test$sp_wd, levels = c(0,0.5,1:7), labels = c(1:9))
data.frame(test$mwbi, test$sp_wd, test$sp_wdF)
t1 <- Sys.time()
pim(formula = mwbi ~ R(sp_wdF) - 1, 
    data = test, compare = "all", link = "identity",vcov.estim = score.vcov)
Sys.time() - t1

pim(formula = mwbi ~ F(sp_wdF) - 1, 
    data = test, compare = "all", link = "identity",vcov.estim = score.vcov)
test %>% mutate(GroupID = paste('G', sp_wdF, sep = '')) %>% spread(key = GroupID, value = sp_wdF)


test %>% select(mwbi, sp_wdF) %>% mutate(GroupID = paste('G', sp_wdF, sep = '')) %>%
  mutate(row = row_number()) %>% select(-sp_wdF) %>%
  gather(vars, val, -GroupID, -row)  %>%
  spread(vars, val)

te2 <- test %>% select(mwbi, sp_wdF) %>% mutate(GroupID = paste('G', sp_wdF, sep = '')) %>% select(-sp_wdF)
reshape(te2, timevar = "mwbi", idvar = "GroupID", direction = "wide")
reshape(te2, direction = "wide")

length(te2$mwbi)
te3 <- data.frame(mwbi = te2$mwbi, model.matrix(mwbi ~ GroupID - 1, data=te2))
colnames(te3) <- c('mwbi', paste('G', 1:9, sep = ''))

# library(reshape2)
# te3 <- 
#   dcast(te2, mwbi ~ GroupID, value.var = 'mwbi')
#         , value.var = 'GroupID')

pim(PO(L(mwbi), R(mwbi)) ~ I(R(G1) - L(G2)), data = te3, compare = 'unique')
pim(PO(L(mwbi), R(mwbi)) ~ I(R(G2) * L(G1)), data = te3, compare = 'unique')
pim(PO(L(mwbi), R(mwbi)) ~ I(R(G3) * L(G1)), data = te3, compare = 'unique', model = "customized")

pim(mwbi ~ G1 + G2, data = te3, compare = 'unique')

pim(PO(L(mwbi), R(mwbi)) ~ I(R(G9) - L(G1)) + I(R(G8) - L(G1)) + I(R(G7) - L(G1)) +
      I(R(G6) - L(G1)) + I(R(G5) - L(G1)) + I(R(G4) - L(G1)) + I(R(G3) - L(G1)) +
      I(R(G2) - L(G1)), data = te3, compare = 'unique')

pim(PO(L(mwbi), R(mwbi)) ~ I(R(G9) * L(G1)) + I(R(G8) * L(G1)) + I(R(G7) * L(G1)) +
      I(R(G6) * L(G1)) + I(R(G5) * L(G1)) + I(R(G4) * L(G1)) + I(R(G3) * L(G1)) +
      I(R(G2) * L(G1)), data = te3, compare = 'unique')

data.frame(mwbi = test$mwbi, sp_wd = test$sp_wd, sp_wdF = test$sp_wdF) %>%
  ggplot(data = ., aes(x = sp_wdF, y = mwbi)) + geom_point() +
  stat_summary(aes(x = as.numeric(sp_wdF), y = mwbi), fun.y = mean, geom = 'line', size = 1)


browseVignettes(package = c("tidyr"))
?spread
pim(PO(L(mwbi), R(mwbi)) ~ I(sp_wdF), data = test, compare = 'unique')


pim(PO(L(Height),R(Height)) ~ I(R(Age) - L(Age)) + I(R(Sex) - L(Sex)),
    data=FEVData, 
    estim = "estimator.BB")

6.289859 * 116.63 / 4 / 60
data.frame(mwbi = test$mwbi, sp_wd = test$sp_wd, sp_wdF = test$sp_wdF) %>%
  ggplot(data = ., aes(x = sp_wdF, y = mwbi)) + geom_point() +
  stat_summary(aes(x = as.numeric(sp_wdF), y = mwbi), fun.y = mean, geom = 'line', size = 1)

data.frame(mwbi = test$mwbi, sp_wd = test$sp_wd, sp_wdF = test$sp_wdF) %>%
  group_by(sp_wdF) %>% summarise(mean(mwbi))
