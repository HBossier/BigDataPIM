...
if(MACHINE == "HPC"){
	...
	write.table(BetaValues, file = paste(ResultsDir, '/leverage_beta_vector_simID_',ID, '_SCEN_', SCEN, '.txt', sep = ''), row.names = FALSE, col.names = TRUE)
	write.table(recordedTime, file = paste(ResultsDir, '/leverage_time_vector_simID_',ID, '_SCEN_', SCEN, '.txt', sep = ''), row.names = FALSE, col.names = TRUE)
}
