...
n <- round(seq(25000,100000,length.out = 5), 0)
u <- 1
alpha <- 5
sigma <- 1
...
for(i in 1:length(n)){
  ...
  write.table(BetaValues[i], file = paste(OutputDir, '/separate/largeN_beta_N_', n[i], '_K_', K, '.txt', sep = ''), row.names = FALSE, col.names = FALSE)
  write.table(recordedTime[i], file = paste(OutputDir, '/separate/largeN_time_N_', n[i], '_K_', K, '.txt', sep = ''), row.names = FALSE, col.names = FALSE)
}

# Save the recorded beta and time values in one file
write.table(BetaValues, file = paste(OutputDir, '/largeN_beta_', K, '.txt', sep = ''), row.names = FALSE, col.names = FALSE)
write.table(recordedTime, file = paste(OutputDir, '/largeN_time_', K, '.txt', sep = ''), row.names = FALSE, col.names = FALSE)
