####################
#### TITLE:     Run resampling scheme for large N: OLS
#### Contents:
####
...
if(is.na(MACHINE)){
  ...
  nsim <- 1000
  ID <- 1:nsim
  ResultsDir <- '/Volumes/1_5_TB_Han_HDD/Mastat/Thesis/BigDataPIM/NonOptimal/LM'
  # Scenario
  SCEN <- 3
}
print(ResultsDir)
...
if(MACHINE == "HPC"){
  ...
  # Fit a linear model nPairs times
  for(i in 1:nPairs){
    # Check progress
    ...
    for(l in 1:nRSloops){
      ...
      # Fit LM. 
      if(SCEN %in% c(1,2,4)){
        LMfit <- try(lm(formula = Y ~ X, data = SelectedData), silent = TRUE)
        if(class(LMfit) == 'try-error'){
          print(paste0('Error in sim ',i, '. Iteration ',l,'.'))
          next
        }else{
          # Save beta value if no error.
          beta_loop <- c(beta_loop, LMfit$coef)
          # Save sandwich variance estimator
          beta_sVar <- c(beta_sVar, summary(LMfit)[,'Std. error'])
        }
      }
      # Different model for scenario 3
      if(SCEN == 3){
        LMfit <- try(lm(formula = Y ~ X_smartph_hrs + X_sex + X_econArea + X_ethnic, data = SelectedData), silent = TRUE)
        if(class(LMfit) == 'try-error'){
          print(paste0('Error in sim ',i, '. Iteration ',l,'.'))
          next
        }else{
          # Names of the objects
          namesObject <- tidy(LMfit$coef) %>% data.table::transpose() %>% slice(.,1)
          
          # Save se
          LMse <- tidy(summary(LMfit)) %>% select(std.error) %>% data.table::transpose()
          if(length(LMse) != 5){
            print(paste0('Error in sim ',i, '. Iteration ',l,'.'))
            next
          }
          names(LMse) <- paste(namesObject, '_se', sep = '')
          beta_sd <- bind_rows(beta_sd, LMse)
          
          # Save beta value if no error.
          LMvalues <- tidy(LMfit$coef) %>% data.table::transpose() %>% slice(.,2)
          names(LMvalues) <- namesObject
          beta_loop <- bind_rows(beta_loop, LMvalues)
        }
      }
    }
    # Recorded time
  ...
  write.table(BetaValues, file = paste(ResultsDir, '/LM_uni_beta_vector_simID_',ID, '_SCEN_', SCEN, '.txt', sep = ''), row.names = FALSE, col.names = TRUE)
  write.table(recordedTime, file = paste(ResultsDir, '/LM_uni_time_vector_simID_',ID, '_SCEN_', SCEN, '.txt', sep = ''), row.names = FALSE, col.names = TRUE)
}
